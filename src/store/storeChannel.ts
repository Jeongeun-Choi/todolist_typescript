import { StoreChannelType, StoreObserverData } from '../../typings/StoreType';
...
class StoreChannel {
  ...
  subscribe(
    channel: StoreChannelType,
    callback: Function,
    bindObj: Object
  ): void {
    let observerDatas: StoreObserverData[] | undefined = this.observers.get(
      channel
    );
  ...
  notify(channel: StoreChannelType): void {
    const observerDatas: StoreObserverData[] | undefined = this.observers.get(
      channel
    );
    ...
    observerDatas.forEach(observerData =>
      observerData.callback.call(observerData.bindObj, data)
    );
  }
}

export const storeChannel = new StoreChannel();
